...
import io.github.alexisTrejo11.construction.company.modules.user.service.AuthService;
import io.github.alexisTrejo11.construction.company.modules.user.service.UserService;
import io.github.alexisTrejo11.construction.company.shared.ResponseWrapper;
import io.github.alexisTrejo11.construction.company.shared.Result;
import io.github.alexisTrejo11.construction.company.shared.dto.auth.LoginDTO;
import io.github.alexisTrejo11.construction.company.shared.dto.auth.UserInsertDTO;
import io.github.alexisTrejo11.construction.company.shared.dto.user.UserDTO;
...
public class AuthController {
  ...
  public ResponseEntity<ResponseWrapper<String>> registerEmployee(@Valid @RequestBody UserInsertDTO userInsertDTO) {
    ...
    if (!credentialsResult.isSuccess()) {
      ResponseWrapper<String> errorResponse = ResponseWrapper.badRequest(credentialsResult.getErrorMessage());
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
    }

    UserDTO userDTO = userService.createUser(userInsertDTO, Role.EMPLOYEE);
    String jwtToken = authService.ProcessRegister(userDTO);

    return ResponseEntity.ok(ResponseWrapper.success(
        jwtToken,
        "user With Role [" + Role.EMPLOYEE.name() + "] " +
  ...
  public ResponseEntity<ResponseWrapper<String>> registerManager(@Valid @RequestBody UserInsertDTO userInsertDTO) {
    ...
    var successResponse = ResponseWrapper.success(
        jwtToken,
        "user With Role [" + Role.MANAGER.name() + "] Successfully Registered"
    );
  ...
  public ResponseEntity<ResponseWrapper<String>> registerAdmin(@Valid @RequestBody UserInsertDTO userInsertDTO) {
    ...
    return ResponseEntity.ok(ResponseWrapper.success(
        jwtToken,
        "user With Role [" + Role.ADMIN.name() + "] Successfully Registered"));
  }
  ...
  public ResponseEntity<ResponseWrapper<String>> login(@Valid @RequestBody LoginDTO loginDTO) {
    ...
    String jwtToken = authService.processLogin(credentialsResult.getData());

    return ResponseEntity.ok(ResponseWrapper.success(jwtToken, "Login Successfully Completed"));
  }
}
